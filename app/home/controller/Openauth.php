...
class Openauth extends Base
{
	...
	public function login($type = null){
        ...
        session('login_http_refer',request()->server('HTTP_REFERER'));
	...
	public function callback($type = null, $code = null){
		...
		if(is_array($token)){
            ...
            if (!empty($userInfo) && $userInfo['user_status']!=1) {
                return ;
            }
            $request = request();
            $ip = $request->ip();
            $refer_url = session('login_refer_url')?session('login_refer_url'):session('login_http_refer');
	        $refer = !empty($refer_url)?$refer_url:'/';
            if (!empty($userInfo)) {
            	Db::name('users')->where($where)->update([
            		'last_login_time'=>time(),
            		'last_login_ip'=>$ip,
            		'user_nicename'=>$user_info['nick'],
                ...
                $data['user_nicename']=$user_info['nick'];
                ...
                $result = Db::name('users')->insertGetId($data);
                if ($result) {
                    $session['uid']=$result;
                    $session['user_nicename']=$user_info['nick'];
